(function () {
  ...
  if (canvas) {
    ...
    // Offscreen canvas for light theme — rain rendered on black bg,
    // then blended onto teal main canvas. Eliminates stripe artefacts.
    ...
    function initMatrix() {
      ...
      // Offscreen same physical size, no DPR scaling needed (we'll scale when blitting)
      offscreen.width  = Math.round(cssW * DPR);
      offscreen.height = Math.round(cssH * DPR);
      octx.setTransform(1,0,0,1,0,0);
      octx.scale(DPR, DPR);
      // Fill offscreen black initially
    ...
    function drawMatrix() {
      ...
      // Both themes: rain is always drawn on a BLACK offscreen canvas.
      // This means shadowBlur glows correctly against black on both themes.
      // For light theme we then blend the offscreen onto the teal main canvas
      // using 'screen' composite — white glow on black becomes additive light on teal.

      // Step 1: fade the offscreen with black semi-transparent rect (creates trails)
      octx.fillStyle = "rgba(0,0,0,0.06)";
      octx.fillRect(0, 0, cssW, cssH);

      octx.font      = FS + "px 'Courier Prime','Cutive Mono',monospace";
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        for (var t = 0; t < tLen; t++) {
          ...
          if (t === tLen - 1) {
            ...
            alpha = 1.0;
            octx.shadowColor = isDark ? "rgba(100,255,140,1)" : "rgba(200,255,220,1)";
            octx.shadowBlur  = 14;
          } else if (t >= tLen - 4) {
            // NEAR-HEAD
            var nf = (t - (tLen - 4)) / 3;
            ...
            alpha = 0.5 + nf * 0.5;
            octx.shadowColor = "rgba(0,255,80,0.6)";
            octx.shadowBlur  = 7;
          } else {
            // TRAIL BODY
            var tf = frac * frac;
      ...
      // Step 2: blit offscreen onto main canvas
      if (isDark) {
        // Dark: offscreen IS the final image — just copy directly
        ctx.clearRect(0, 0, cssW, cssH);
        ctx.drawImage(offscreen, 0, 0, cssW, cssH);
      } else {
        // Light: fill main with solid teal bg, then screen-blend the rain on top
        // 'screen' composite: dark areas of offscreen (black) = transparent,
        // bright green/white glow = luminous overlay on teal — exactly like
        // holding a green-lit transparency over a teal surface.
        ctx.fillStyle = "rgb(" + bgArr[0] + "," + bgArr[1] + "," + bgArr[2] + ")";
  ...
  /* ── Draggable music float ──────────────────────────────────────────── */
  if (mFloatEl && mFloatBar) {
    var dragState = { active: false, startX: 0, startY: 0, origLeft: 0, origTop: 0 };

    function musicDragStart(clientX, clientY) {
      var rect = mFloatEl.getBoundingClientRect();
      // Switch from bottom/left CSS to explicit top/left so we can drag freely
      mFloatEl.style.bottom = "auto";
      mFloatEl.style.right  = "auto";
      mFloatEl.style.left   = rect.left + "px";
      mFloatEl.style.top    = rect.top  + "px";
      dragState.active  = true;
      dragState.startX  = clientX;
      dragState.startY  = clientY;
      dragState.origLeft = rect.left;
      dragState.origTop  = rect.top;
      mFloatEl.classList.add("is-dragging");
    }

    function musicDragMove(clientX, clientY) {
      ...
      var maxLeft = window.innerWidth  - mFloatEl.offsetWidth;
      var maxTop  = window.innerHeight - mFloatEl.offsetHeight;
      newLeft = Math.max(0, Math.min(maxLeft, newLeft));
      newTop  = Math.max(0, Math.min(maxTop,  newTop));
      mFloatEl.style.left = newLeft + "px";
      mFloatEl.style.top  = newTop  + "px";
    }

    function musicDragEnd() {
      if (!dragState.active) return;
      dragState.active = false;
      mFloatEl.classList.remove("is-dragging");
    }

    // Mouse
    mFloatBar.addEventListener("mousedown", function (e) {
      if (e.target === mFloatPlay || (mFloatPlay && mFloatPlay.contains(e.target))) return;
      if (e.target === mFloatToggle || (mFloatToggle && mFloatToggle.contains(e.target))) return;
      e.preventDefault();
      musicDragStart(e.clientX, e.clientY);
    });
    document.addEventListener("mousemove", function (e) { musicDragMove(e.clientX, e.clientY); });
    document.addEventListener("mouseup",   musicDragEnd);

    // Touch
    mFloatBar.addEventListener("touchstart", function (e) {
      if (e.target === mFloatPlay || (mFloatPlay && mFloatPlay.contains(e.target))) return;
      if (e.target === mFloatToggle || (mFloatToggle && mFloatToggle.contains(e.target))) return;
      var t = e.touches[0];
      musicDragStart(t.clientX, t.clientY);
    }, { passive: true });
    document.addEventListener("touchmove", function (e) {
      if (!dragState.active) return;
      var t = e.touches[0];
      musicDragMove(t.clientX, t.clientY);
    }, { passive: true });
    document.addEventListener("touchend", musicDragEnd);
  }
