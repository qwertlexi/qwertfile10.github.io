(function () {
  ...
  if (canvas) {
    ...
    function drawMatrix() {
      ...
      // Fade rect — higher alpha on light theme to fully erase previous frame
      // without leaving teal column-stripe artefacts from shadowBlur accumulation
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        for (var t = 0; t < tLen; t++) {
          ...
          if (t === tLen - 1) {
            ...
            alpha = 1.0;
            if (isDark) { ctx.shadowColor = "rgba(100,255,140,1)"; ctx.shadowBlur = 14; }
          } else if (t >= tLen - 4) {
            // NEAR-HEAD
            var nf = (t - (tLen - 4)) / 3;
            ...
            alpha = 0.5 + nf * 0.5;
            if (isDark) { ctx.shadowColor = "rgba(0,255,80,0.6)"; ctx.shadowBlur = 7; }
          } else {
            // TRAIL BODY
            var tf = frac * frac;
      ...
      // ── Light theme glow pass ──────────────────────────────────────
      // Draw head + near-head chars a SECOND time with shadowBlur only.
      // Keeping this separate from the main pass prevents the stripe artefact
      // that occurs when shadowBlur bleeds across adjacent column boundaries.
      if (!isDark) {
        ctx.save();
        for (var gi = 0; gi < cols.length; gi++) {
          var gc  = cols[gi];
          var gx  = gi * FS;
          var tL  = gc.trail.length;
          if (tL === 0) continue;
          // Head glow
          var gyH = gc.y;
          if (gyH >= -FS && gyH <= cssH) {
            ...
            ctx.fillText(gc.trail[tL - 1], gx, gyH);
            ctx.shadowBlur  = 0;
          }
          // Near-head glow (2 chars)
          for (var ng = 1; ng <= 2; ng++) {
            if (tL - 1 - ng < 0) continue;
            var gyN = gc.y - ng * FS;
            if (gyN < -FS || gyN > cssH) continue;
            ctx.shadowColor = "rgba(255,255,255,0.5)";
            ctx.shadowBlur  = 8;
            ctx.fillStyle   = "rgba(240,255,255,0.7)";
            ctx.fillText(gc.trail[tL - 1 - ng], gx, gyN);
            ctx.shadowBlur  = 0;
          }
        }
        ctx.restore();
    ...
    setTimeout(function () {
      ...
      if (tarotCardImg) {
        ...
        if (customFrontImages[idx]) {
          ...
          img.src = customFrontImages[idx]; img.alt = card.cn;
  ...
  if (rainToggleBtn) {
    rainToggleBtn.addEventListener("click", function () {
      ...
      if (rainTag) rainTag.textContent = rainEnabled ? "ON" : "OFF";
    });
  }
